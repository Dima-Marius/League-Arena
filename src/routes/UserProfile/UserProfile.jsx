...
const UserProfile = () => {
  ...
  useEffect(() => {
    if (summonerId !== null && summonerId !== undefined) {
      fetch(
        `https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/${summonerId}/ids?start=0&count=5&api_key=${API_KEY_CTX.apiKey}`
      )
        .then((response) => response.json())
      ...
      <div className={style.container}>
        ...
        <div className={style.main}>
          ...
          <div className={style.info}>
            <p>
              Username: <span>{currentUser?.ign}</span>
            </p>
            <p>
              Region:{" "}
              <span>
            ...
            <p>
              ...
              <span className={style["role-icon"]}>
                <img
                  src={
                    currentUser.length !== 0 && getRoleIcon(currentUser?.role)
                  }
                  width="50px"
                  height="50px"
                  alt="role"
          ...
          <div className={style.details}>
            <div>
              <h2>
                Rank{" "}
                <span className={style["ranking-star"]}>
            ...
            <div>
              <img
                src={currentUser.length !== 0 && getRankIcon(currentUser?.rank)}
                width="200px"
                height="200px"
                alt="rank"
              />
          ...
          <div className={style.about}>
            ...
            <p>
              ...
              deserunt minus voluptatibus officiis id, enim, autem nobis eius
              quasi animi fuga hic optio repudiandae explicabo obcaecati vitae
              debitis corrupti aliquid dolorem eligendi quisquam quis quaerat
              deleniti iure! Natus, exercitationem quos maxime nesciunt
              repudiandae suscipit alias eius officiis excepturi quisquam velit
              odit sint, quidem voluptates quod ut laborum, hic minus sequi
              ullam eligendi accusamus fugit! At praesentium maxime voluptatem
              illo maiores doloribus tenetur aperiam quos corrupti veniam. Nulla
              doloremque voluptates placeat rerum velit quia animi distinctio
              aperiam inventore? Voluptates aliquid recusandae quaerat eos ad
              eveniet veritatis animi perspiciatis, inventore officiis pariatur.
            </p>
          </div>
                ...
                <div className={style["team-record"]}>
                  ...
                  <div className={style.record}>
                    <div>
                      <p>
                        Wins:
                        <span className={style.wins}>{userTeam?.wins}</span>
                      </p>
                      <p>
                        Losses:
                        <span className={style.losses}>{userTeam?.losses}</span>
                      </p>
                    </div>
                    <p>
                      Winrate:
                  ...
                  <div className={style["members-container"]}>
                    ...
                    <ol className={style.members}>
                      {userTeam?.members?.map((member) => (
                        <TeamMember key={member} memberName={member} />
                      ))}
                    </ol>
                  </div>
                </div>
          ...
          <div className={style.stats}>
            ...
            <ul className={style["match-history"]}>
              {summonerMatches?.map((match) => (
                <MatchItem key={match} currentSummoner={ign} matchId={match} />
              ))}
            </ul>
          </div>
        </div>
  ...
  if (userProfileExists === false) return <NotFound />;
