...
const MatchItem = ({ matchId, currentSummoner }) => {
  ...
  const [matchData, setMatchData] = useState(
    JSON.parse(localStorage.getItem(matchId)) ?? {}
  );
  ...
  return (
    <li className={style.li}>
      ...
      <div className={style["score-container"]}>
        <p>
          <span className={style.score}>Score:</span> {matchData?.kills} /{" "}
          {matchData?.deaths} / {matchData?.assists}
        </p>
      </div>
      <div>
      ...
      <div className={style.verdict}>
        {matchData.win ? (
          <p className={style.victory}>Victory</p>
        ) : (
          <p className={style.defeat}>Defeat</p>
        )}
      </div>
      <div className={style.role}>
        <img
          src={getRoleIcon(matchData?.teamPosition)}
          width="35px"
          height="35px"
          alt=""
        />
